...
if __name__ == '__main__':
    ...
    features_filtered = select_features(extracted_features, Y)  # 用select_features函数过滤特征
    ...
    features_filtered_test = extract_features(data_format_test, column_id="id", column_sort="time", kind_to_fc_parameters=kind_to_fc_parameters)
    ...
    params = {'num_leaves': 9,  # 一棵树中最大的叶子数 ，默认31,影响很大
              'objective': 'regression',  # 用LGB回归模型
              'max_depth': 4,   # 树的最大深度 当模型过拟合时，需要首先考虑
    ...
    for fold_, (trn_idx, val_idx) in enumerate(folds.split(features_filtered, Y)):
        ...
        print(auc)
        auc_score += auc
        preds = clf.predict(features_filtered_test, num_iteration=clf.best_iteration)  # 指定选用最好的迭代次数。
        test_result += preds
    ...
    df.to_csv("baseline23.csv", index=False)
